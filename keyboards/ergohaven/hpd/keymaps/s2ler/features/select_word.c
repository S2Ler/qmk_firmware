#include "select_word.h"

//#define DEBUG_SELECT_WORD

enum {
    STATE_NONE,       // No selection.
    STATE_SELECTED,   // Macro released with something selected.
    STATE_WORD,       // Macro held with word(s) selected.
    STATE_FIRST_LINE, // Macro held with one line selected.
    STATE_LINE        // Macro held with multiple lines selected.
};

static uint8_t state = STATE_NONE;

void select_word_debug(void);

bool process_select_word(uint16_t keycode, keyrecord_t* record, uint16_t sel_keycode) {
    if (keycode == KC_LSFT || keycode == KC_RSFT) {
        return true;
    }

    if (keycode == sel_keycode && record->event.pressed) { // On key press.
        const uint8_t mods = get_mods();
        const bool shifted = (mods | get_oneshot_mods()) & MOD_MASK_SHIFT;
        const bool ctrled = (mods | get_oneshot_mods()) & MOD_MASK_CTRL;
        clear_oneshot_mods();

        if (shifted) { // line select
            if (state == STATE_NONE) {
                // Tap GUI (Command) + Left, then Shift + GUI + Right.
                set_mods(MOD_BIT(KC_LGUI));
                send_keyboard_report();
                tap_code(KC_LEFT);
                register_mods(MOD_BIT(KC_LSFT));
                tap_code(KC_RGHT);

                set_mods(mods);
                state = STATE_FIRST_LINE;
            } else {
                register_code(KC_DOWN);
                // Tap Shift+Ctrl+E to select line to the end
                register_mods(MOD_BIT(KC_LCTL) | mods);
                tap_code(KC_E);
                unregister_mods(MOD_BIT(KC_LCTL));
                state = STATE_LINE;
            }
        }
        else if (ctrled) {
            if (state == STATE_NONE) {
                // ctrl+gui+u
                register_mods(MOD_BIT(KC_LGUI));
                tap_code(KC_P);
                unregister_mods(MOD_BIT(KC_LGUI));
            }
        }
        else { // world select
            set_mods(MOD_BIT(KC_LALT));
            if (state == STATE_NONE) {
                // Move to the start of the word
                send_keyboard_report();
                tap_code(KC_RGHT);
                tap_code(KC_LEFT);
            }
            // then select it.
            register_mods(MOD_BIT(KC_LSFT));
            register_code(KC_RGHT);
            state = STATE_WORD;
        }
        select_word_debug();
        return false;
    }

    // `sel_keycode` was released, or another key was pressed.
    switch (state) {
        case STATE_WORD:
            unregister_code(KC_RGHT);
            unregister_mods(MOD_BIT(KC_LSFT) | MOD_BIT(KC_LALT));
            state = STATE_SELECTED;
            break;

        case STATE_FIRST_LINE:
            state = STATE_SELECTED;
            break;

        case STATE_LINE:
            unregister_code(KC_DOWN);
            state = STATE_SELECTED;
            break;

        case STATE_SELECTED:
            if (keycode == KC_ESC) {
                tap_code(KC_RGHT);
                state = STATE_NONE;
                return false;
            }
            // Fallthrough intended.
        default:
            state = STATE_NONE;
    }
    select_word_debug();

    return true;
}

void select_word_debug(void) {
#ifdef DEBUG_SELECT_WORD
    switch (state) {
        case STATE_NONE:
            debug_msg("Feature(SELECT_WORD): STATE_NONE\n");
            break;
        case STATE_SELECTED:
            debug_msg("Feature(SELECT_WORD): STATE_SELECTED\n");
            break;
        case STATE_WORD:
            debug_msg("Feature(SELECT_WORD): STATE_WORD\n");
            break;
        case STATE_FIRST_LINE:
            debug_msg("Feature(SELECT_WORD): STATE_FIRST_LINE\n");
            break;
        case STATE_LINE:
            debug_msg("Feature(SELECT_WORD): STATE_LINE\n");
            break;
        default:
            debug_msg("Feature(SELECT_WORD): Unknown state\n");
            break;
    }
#endif
}