...
#define EN_LANG_IDX 1
#define RU_LANG_IDX 2
...
void process_combo_event(const uint16_t combo_index, const bool pressed) {
    switch (combo_index) {
        case NEW_LINE_COMBO:
            if (pressed) {
                tap_new_line();
...
uint8_t get_language_idx(void) {
    if (IS_LAYER_ON(HOME_LAYER_RU)) {
        return 2;
    } else if (IS_LAYER_ON(HOME_LAYER_EN)) {
        return 1;
    }
    return 1;
}

void set_language_idx(const uint8_t lang_idx) {
    if (lang_idx == 1) {
        layer_off(HOME_LAYER_RU);
        layer_on(HOME_LAYER_EN);
        layer_off(SYMBAL_LAYER_RU);
        layer_on(SYMBAL_LAYER_EN);
    } else if (lang_idx == 2) {
        layer_off(HOME_LAYER_EN);
        layer_on(HOME_LAYER_RU);
        layer_off(SYMBAL_LAYER_EN);
        layer_on(SYMBAL_LAYER_RU);
    }
}
